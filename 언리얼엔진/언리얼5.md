...
1. 스프라이트 만들기
   1. Objcts폴더/Strikes폴더 만들기
   2. 하던대로 작업 create sprite
2. 블루프린트 뷰
   1. 2DPlatformerBluprints폴더에 블루프린트 클래스 만들기.  Actor 이름은 Sprikes
   2. paperSprite 컴포넌트 추가.  위에서만든 스프라이트 매핑
   3. Box Collision 컴포넌트 추가
   4. 크기 조정하고
   5. Paper Sprite의 Collision Presets는 NoCollision 설정
   6. Box에서 overlap이벤트 추가버튼
3. 블루프린트 이벤트
   1. Begin Overlap의 상대방이 캐릭터면 그놈에게 데미지 주기
      1. CastTo2dCharacter - Apply Damage
         1. As 2d Character는 Damaged Actor와 연결
         2. Base Damage는 변수등록.  SpikedDamage   기본값1

>42. Setting Up a Background For Are Level(7:29)
1. 리소스 가져오기
   1. Tileset폴더를 Content폴더 하위로
   2. BG1 로 create sprite 하면 초록색으로 만들어지는데 다시 이 이미지만 가져오고 잠시 뜬 창에서 revert하면 된다.
   3. 2dSprite로 바꾸기
   4. BG1만 다시 create sprite
2. 화면에 적용
   1. 화면으로 드래그
   2. y축으로 -100
   3. scale은 비율유지하며 10
   4. 적절히 배치
3. 타일셋 - 타일맵 만들기
   1. 배경이미지 - 우클릭 - create tileSet
   2. 방금가져온 BG1, BG2, BG3 배경이미지에 다 적용
   3. 만들어진 타일셋 우클릭 - create tilemap
   4. 타일맵 더블클릭 들어가기
      1. 타일모양 클릭해서 풀모양 타일셋선택
      2. 왼쪽영역에서 영역지정하고 ctrl + c 한다음 오른쪽에서 클릭
      3. 하늘영역가서 paint버튼 클릭하고 드래그
      4. Eraser, Fill 연습
   5. 브러쉬해서 전체를 타일맵으로 이동 (우리는 현재 타일맵 크기 10, 6 하면 맞다.)
   6. 오른쪽위 Layer1 이름바꾸기  Skybackground
   7. 새레이어추가  (새 레이어가 위로 덮어쓴다.)
      1. 왼쪽 액티브타일셋 BG2로 변경
      2. 전체가져와서 오른쪽에 찍고, 이름 Clouds
   8. 새레이어추가
      1. BG3 풀 다 가져와서 오른쪽에 찍고 이름 grasslayer
   9. save
4.  타일맵 레벨에 드래그
5.  이걸로 보기좋게 크기조정
6.  월드에 있던 Atmospheric Fog제거 - 까매짐


> 43. Tilesets(4:55)
디자이너가 타일셋을 만들고, 개발자나 기획자는 만들어진것으로 맵디자인을 할 수 있다.
1. Tileset폴더의 Tileset파일 우클릭 타일셋으로 만들고 더블클릭
2. 타일사이즈가 32, 32 되어있는걸 16, 16으로 변경
3. //원하는곳 선택후 상단 Add Box, Add Poligon, Add Circle 설명
4. 선택후 Colliding Tiles버튼 체크한후  (충돌지점 설정)
   1. 땅에 풀나있는 타일들 각각 클릭하고 Add Box 버튼 누르기 반복해서 실행
   2. save
5. 컨텐츠 브라우저로 나와서 타일셋 우클릭 - Create TileMap  이름은 level1tilemap
   1. 더블클릭으로 진입
   2. Map Width 20, Map Height 6으로 변경
   3. 큰땅2개, 작은땅1개 배치시키고
   4. 맵에 로드해서 y위치 0, 스케일5로 해서 보기좋은곳에 위치
   

> 44. Sett Up Custom Projectiles(3:04) 고블린은 폭탄을 던지게 변경
1. RangedEnemy 배치. 타입은 고블린으로 변경
2. 블루프린트 진입
   1. Enemy Attacking에서 SpawnActor 부분 클래스를 우클릭해서 변수로뽑기. 변수명 : EnemyProjectile
   2. Instance Editable, Expose on Spawn 체크
3. Enemies/Goblin/Projectile 폴더에 Bomb_sprite가져오기//나는 예전에 RangedAttack폴더에 잘못붙여넣었었기 때문에
   1. paper2d로 바꾸고
   2. extract grid 100하고
4. Enemy_BP의 Mushroomprojectile 복제.  이름 : bombprojectile
   1. 들어가서 뷰포트. 
   2. 스프라이트를 폭탄 0으로 변경하고 위치조정
   3. 이벤트 그래프에서 ApplyDamage에 있는 기본데미지 3으로 변경
5. 맵에 원거리 고블린 선택해서 Enemy Projectile에서 bombprojectile 선택


> 45. Enemy Fixes (적 수정)
시체(죽은뒤에) 동전나오고 충돌 없애기
1. RangedEnemy 이벤트 수정
   1. handleDamage부분
         1. Enemy Dead Body?로 set해주는곳 뒤에 SpawnActor from Class
         2. 클래스는 Coin
         3. 돋보기 버튼누르면 콘텐츠 브라우저에 빨리찾을수있도록 선택된다.
         4. Coin 복사해서 EnemyCoin으로 만들기
         5. 이벤트그래프에서 ++ 없애고 add함수 연결
         6. add의 밑은 변수추가. RandomCoinNo라고 이름정하고 Instance Editable, Expose on Spawn체크
         7. 연결할것들 하고 컴파일
      1. 클래스를 EnemyCoin으로 변경
         1. RandomCoinNo 끌어서 random integer in range 연결
         2. 1부터 10까지로 지정
      2. Spawn Transform은 쪼개기
         1. 위치와 getActorLocation연결
      3. Set과 SpawnActor Enemy 사이에 간격 조금 벌리고 캡슐컴포넌트 가져와서 Set Collision Profile name
         1. 연결연결
         2. 캡슐컴포넌트의 Details의 collision.  Pawn인상태. OverlapOnlyPawn도 있는거 확인하고
         3. Set Collision Profile name을 OverlapOnlyPawn 으로 설정
      4. 테스트(3:50) : 원거리는 이제 시체 충돌안하는데 근거리는 여전히 충돌함. 죽으면 코인이1개 나오는데 먹으면 올라가는 숫자는 랜덤임

2. 시체도 캐릭터 방향으로 쳐다보는거 해지
   1. Handle Rotation부분 변경
      1. Branch앞에 Enemy Dead상태인지 체크해서 아닐때만 원래하던일
3. 했던것들 복사해서 근접 Enemy에게도 붙여넣기
   1. 추가로 Enemy Attacking의  LineTraceForObjects앞에 적이 죽었는지 체크하는 Branch False일때 원래하던걸로 연결
   2. Ranged Enemy에도 공격하는곳의 SpawnActor 앞에 죽었는지 체크하는거 넣어주기
4. 테스트

> 46. Creating A Health Pick Up
1. Sprites/Hearts에 있는 빨간하트표시 스프라이트로 생성
2. 블루프린트생성 - Actor로.  이름은  Heartpickup
   1. paperSprite 컴포넌트 추가. 크기5으로 변경.   Collision은 NoCollision으로
   2. Box Collision 컴포넌트 추가 . 크기조정
   3. Box에서 2번째 이벤트 추가
3. On Component Begin Overlap
   1. Other Actor : Cast to 2dCharacter
      1. //As 2d Character 는 타겟의 헬스에 플러스할 하트양만큼 더하기
      2. //get Health, Add(),  변수추가(이름은 health.  instnace Editable, Expose on Spawn체크. )
      3. 기본값 1. 컴파일
   2. CastTo2dCharacter의 As2dCharacter에서 한줄 더 빼기. Set Health
   3. add결과와 Set Health 사이에 Clamp(float)추가. 
      1. As 2d Character에서 get Maxhealth가져오기. Max와 연결
      2. Min은 1
      3. Set에 다 연결시키고 테스트
   4. (3:51)  Set 뒤에 destroyActor(self)    //충돌된 하트는 없애기
   5. Playerhud_WB 의 Graph탭  //잘이해안됨
      1. UpdateHealth에 있는 RemoveHealth 더블클릭하면 Heart_WB로 간다. //보기만하네
      2. Remove앞에 끊고, 변수Heart Ref 복제. 이름은 Removedhearts
      3. 드래그해서 ADD연결. 앞에 있는 For Each Loop의 Array Element와 연결
   6. Heart_WB
      1. 우클릭 add Custom Event.  이름은 showHeart
         1. 위의 SetVisibility복제해서 연결
         2. in Visibility는 Visible로 변경
   7. 다시  Playerhud_WB
      1. 우클릭 add Custom Event.  이름은 ShowHeart
      2. for Loop
         1. Last Index를 ShowHeart로 연결
   8. HeartPickup (6:20)
      1. CastTo2dCharacter의 As2dCharacter 뽑아서 getPlayerHud - ShowHeart
         1. Set의 마지막과 ShowHeart연결
         2. get Health를 Last Index에 연결
         3. ShowHeart의 끝을 DestroyActor와 연결
   9. Playerhud_WB로 와서 빼야될 하트 계산
      1. Removedhearts 드래그
      2. LENGTH - 1빼기(subtract 1)
      3. get(a copy) - show Heart
      4. GET과 위에 있는 For Loop를 Show Heart의 인풋으로 연결
      5. ADD  input2개(get Heart Ref,  앞의 GET)
      6. REMOVE input2개(get Removedhearts, 앞의 GET)
      7. 컴파일, 테스트 : 1회복하고 하트없어짐,  풀체력을 넘어선 체력회복은 에러뜸
   10. 버그수정
       1.  Plaeryhud_WB에서 showHeart의 GET과 ShowHeart 사이에 isValid


> 47. Adding Sound Effects(9:52)
1. 에셋에서 SoundEffects폴더를 우리 Content밑으로 복붙
2. 도끼
   1. 사운드 확인. 
   2. 도끼줍기 : Axe_Pickup의 컴퍼넌트비긴의 Set 뒤에  Play Sound 2D 연결.
      1. pickupAxe 선택
      2. 테스트. 이것만 하니까 소리 나네
   3. 도끼던지기 : 2dCharacter의 InputAction ProjectileAttack - SpawnActor Axe뒤에 Play Sound 2D 연결.
      1. throwAxe 선택
3. 코인
   1. 우클릭 Sounds - SoundCue  이름은 Coinsoundcue
   2. 더블클릭
   3. coin1사운드부터 9까지 드래그앤드랍으로 가져오기
   4. coin1에서 Random 연결
   5. Add input 늘려서 모든 코인과 연결
   6. 아웃풋과 연결
   7. 코인 블루프린트
      1. 마찬가지로 DestroyActor앞에 Play Sound 2D 연결.  Coinsoundcue매핑
   8. EnemyCoin 블루프린트 마찬가지
4. Health 도 ShowHeart앞에서 PlaySound2d
5. Sword
   1. 우클릭 Sounds - SoundCue  이름은 SwordHit_Cue
   2. 더블클릭
   3. 1부터4까지 드래그앤드랍
   4. 마찬가지로 랜덤 연결
6. Swoosh
   1. 2dCharacter블루프린트
   2. MeleeAttack부분  맨끝 Apply Damage 앞의 Breat Hit Result의 Hit Actor에서 뽑기
   3. is Valid - True면 playSound2d - SwordHit_Cue,   False면 헛스윙. playSound2d - Swoosh
7. 7:20 테스트시 딜레이 발생
   1. Dealy앞에다가 연결
      1. 근데 클릭할때마다 슉슉슉슉함.
      2. Set IsAttacking? 앞의 거리 벌리기  branch뒤에 doOnce연결
      3. Dealy뒤의 Sequence의 Then2를 DoOnce의 Reset과 연결
8. Hurt
   1. RangedEnemy의  EventAnyDamage의 SetEnemydagamed?오른쪽에 play sound hurt로 연결

> 49. Designing A Level
1. Sprites/Tileset/
   1. Decors(나무모양) - 우클릭 - create tileset
   2. level1tilemap 더블클릭
      1. 설정된거 다 eraser로 지우기
      2. 가로100, 세로10 으로 크기 조정
      3. 오른쪽 Layer1 이름 Main으로 변경
      4. 땅 다양하게 재미있게 채우기
      5. 레이어 하나 추가.  이름은 treebackground.  Main보다 밑으로 내리기
      6. 왼쪽 ActiveTileSet에서 Decors Tileset 선택
         1. 나무선택하면 타일크기가 안맞음. 타일셋들어가서 32인거 16으로 변경
      7. 레이어하나더 추가. 이름은 bushbackground. 나무보다 아래로 내리기
   3. 만든 타일셋 레벨에 배치. 크기5로 변경
   4. 테스트
   5. Main높이 20으로 변경해서 어두운땅으로 채우기
2. 맵바깥으로 가면 추락하는것 수정할것임
   1. Cube를 왼쪽에 세로로 길게 갖다붙이고 Details에서 Actor Hidden in Game 체크
   2. 복사해서 오른쪽벽에도 갖다놓기
3. 여러 적들 배치
   1. 데미지, canmove 등 인스턴스에서 조정
...
### 웨비나
https://www.youtube.com/watch?v=CfpJEx-3hYg&ab_channel=UnrealEngineKR
1. 에디터 소개
2. 게임의 승패조건
   1. 게임모드 : 아웃라이너 제일상단 레벨 우클릭 - 월드세팅 - Game Mode 1:23
   2. 테스트노하우 : 액터의 위치를 시작할곳으로 이동하여 실행