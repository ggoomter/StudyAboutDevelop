...
# MVC
  ...
  - 우선 가장중요한 개념은 LAYER
    - presentation(화면에 보여지는), Service(프로세스 처리), Business(업무), Persistence(영속성)
    - 레이어를 분리한다는것은 추상화하여 부품갈아끼우듯이 할수있는 것
    - Model은 DTO가 아니다. 그건 안티패턴. ★★state와 logic의 조합★★
  -용어 설명(집 공사를 예제로)
    - Model  : 결과를 만들때 필요한 데이터를 준비(창문, 전등, 싱크대, 냉장고, 재료) = DTO, VO
...
쉽게말하면 URI로 리소스를 정의(자원의 표현)하고, HTTP메소드로 리소스에 대한 행위를 정의한 것
1. 자원의 식별
    URI에 식별자들이 들어있어야 한다.
2. 메세지를 통한 리소스 조작
