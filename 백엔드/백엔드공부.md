...
##### 대칭키
동일한 키로 암호화와 복호화를 같이 하는 방식을 말한다.
예를들어 1234로 암호화를 했으면 복호화를 할때도 1234 사용
단점 : 암호를 주고받는 사람들사이에 대칭키를 전달하는 것이 어렵다.
      대칭키가 유출되면 키를 가지고있는 사람들은 어떤 값이든지 복호화 할 수 있기 때문에
##### 공개키
대칭키 방식을 해결하고자 나온 방식.
타인에게 제공하는 공개키와 자기만 가지고 있는 비공개키가있다.
공개키를 가지고 있는 타인은 공개키를 사용하여 암호화하고, 나는 비공개키로 복호화한다.
공개키로는 암호화는 할수 있지만 복호화는 할 수 없기 때문에 더 안전하다.
공개키가 데이터를 제공한 사람의 신원을 보증하는 역할을 한다. 이것을 전자서명이라고 한다.
- 대표 : RSA, SSL
