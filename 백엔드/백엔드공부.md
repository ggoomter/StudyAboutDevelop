...
- 톰캣폴더/conf/server.xml의 Context 태그에 추가하면된다.
  - 이클립스에서 바로 앱을 서버로 연동시킬경우 프로젝트 이름으로 자동 Context를 생성해준다.
...
- PRG패턴(Post - Redirect - Get) https://sweets1327.tistory.com/52
...
1. 프로젝트에 Firebase추가하는 과정
  ...
  (
  - AndroidManifest.xml파일의 manifest항목의 package속성과 등록할때 패키지이름은 같아야한다.
...
#### URI : Uniform Resource Identifier. 통합자원식별자 (논리적 위치)
  ...
  뜻 :  https 로 get요청을 한다. www.fastcampu.co.kr이라는 도메인을 가진 서버에게.
    그 서버의 80포트를 리스닝하고 있는 웹서버에게 resources하위의 sample 하위의 1번을 요청한다. 이 요청이 들어왔을때 반환할 응답을 지정해놓는다.
  상도동/2번째사거리/3번째건물/3층/첫번째방
#### URL : Uniform Resource Locator
  인터넷상의 특정 리소스가 어디있는지 위치를 알려주기위한 규약. 프로토콜이 포함된다.
  식별+자원의위치
  예) https://www.fastcampus.co.kr/resources/files/fastcampus.pdf (물리적 위치)
  뜻 : https 로 get요청을 한다. www.fastcampu.co.kr이라는 도메인을 가진 서버에게.
    그 서버의 80포트를 리스닝하고있는 웹서버의 루트폴더에 있는 fastcampus.pdf 를 달라.
  상도동/상도2가-1/304의201호

- URI설계원칙 RFC-3986
1. /를 계층관계를 나타내는데 사용한다.
2. 마지막에는 / 사용하지 않는다.
3. 리소스는 동사가 아니라 명사를 사용해야 한다.
4. _가 아닌 -를 쓴다.
5. URI는 소문자로만 작성
6. 파일의 확장자는 포함하지 않는다.
7. 프로그래밍 언어에 의존적인 확장자를 사용하지 않는다.  예).do
8. 구현에 의존적인 경로를 사용하지 않는다. 예)servlet/classes/
9. 세션id를 포함하지 않는다.
10. 프로그래밍언어의 메소드명을 이용하지 않는다.
11. 복수형 명사 사용. 특히 컬렉션은
12. 컨트롤러이름으로는 동사나 동사구를 사용한다.
13. 경로중 변하는 부분은 유일한 값으로 대체한다.
14. CRUD 기능을 나타내는것은 URI에 사용하지 않는다.
15. URI 쿼리로 컬렉션 결과에 대해서 필터링 할 수 있다.
16. URI쿼리는 컬렉션의 결과를 페이지로 구분하여 나타내는데 사용한다.
...
- 필터와 인터셉터의 결정적인 차이는 
필터는 스프링과 무관한 서블릿 자원이고,
인터셉터는 스프링의 빈으로 관리되며 스프링의 컨텍스트 내에 속한다는 것.
때문에 인터셉터는 스프링 컨텍스트 내에 있는 자원을 활용 할 수 있다.
...
- Advice(부가기능) :  부가기능을 정의한 코드. 5가지 종류
    - @Before: 타겟 메서드가 실행(호출)되기 이전에 처리해야될 필요가 있는 부가기능
    - @After: 대상 메서드의 수행(호출) 후
    - @After-returning: 대상 메서드의 정상적인 수행(호출) 후
    - @After-throwing: 예외발생 후ㆍ
    - @Around: 타겟 메서드의 실행(호출) 이전, 이후에 모두 처리해야 될 필요가 있는 부가기능
    //사실 Around를 제외한 나머지 어드바이스들은 Around가 할수있는 일의 일부만 제공할 뿐이다. Around는 JoinPoint를 상속받은 ProceedingJoinPoint를 사용해야한다.
    @Around는 항상 조인포인트.proceed()를 호출해야한다.
    (**유의**)Around를 썼는데 proceed()한 오브젝트를 바로 리턴해버리면 함수실행이후에 실행되어야할 aop가 실행안되는것에 유의
...
- ACID원칙
  > 다수의 트랜잭션이 경쟁시 발생할수 있는 문제를 방지하기 위함
  ...
  - Isolation(격리) : **격리수준**은 트랜잭션에 일관성이 없는 데이터를 허용하는 수준을 말한다.
  DEFAULT, READ_UNCOMMITTED, READ_COMMITTED, REPEATABLE_READ, SERIALIZABLE
  - Durability(영속성) : 성공적으로 처리되면 그 결과는 영속성으로 보관되어야 한다.

- propagation(프로파게이션. 전파옵션) 속성
  ...
  ▶ NEVER - 트랜잭션을 사용하지 않도록 강제한다. -0 이미 진행 중인 트랜잭션도 존재하면 안된다 있다면 예외를 발생시킨다.
  ▶ NESTED - 이미 진행중인 트랜잭션이 있으면 중첩 트랜잭션을 시작한다. - 중첩 트랜잭션은 트랜잭션 안에 다시 트랜잭션을 만드는 것이다.

- readOnly 속성
  - 트랜잭션을 읽기전용으로 설정한다.
  - 트랜잭션 시작후 INSERT, UPDATE, DELETE 같은 쓰기작업이 진행되면 예외가 발생한다.
  - 성능을 최적화 하거나 쓰기작업을 방지하기 위해 사용한다.

- timeout 속성
  - 지정한 시간내에 해당메소드 수행이 완료되지 않은 경우 롤백 수행
...
- 스프링부트에서의 트랜잭션
  - @Transactional이 추가되면, 이 클래스에 트랜잭션 기능이 적용된 프록시 객체가 생성된다
  - 이것을 선언적 트랜잭션이라고 한다.
  - 런타임 예외가 발생하면 롤백하지만 예외가 없거나 체크예외가 발생하면 커밋한다.
  - 예외가 발생했는데 롤백이 안되는 이유로 설계한 이유는 스프링프레임워크는 EJB의 관습을 따르기 때문이다.
  하지만 checked exception의 경우에도 롤백 시켜야될 상황이 있다면 어떻게 해야할까?    rollbackOn속성 이용
  예)  @Transactional(rollbackOn = {Exception.class})

- Junit테스트에서 예외없는데 db반영 안되는 이유
...
- [프로그래머스 쓰레드 간단구현](https://www.youtube.com/watch?v=jcbIKHFhGIg&t=44)
- [프로세스, 멀티쓰레드, 멀티태스킹, 멀티프로그래밍](https://www.youtube.com/watch?v=QmtYKZC0lMU)
...
- 대표적인 메소드
  ...
  쓰레드를 start하면 바로 실행하는것이 아니라 실행대기상태에 있다가 OS의 스케쥴러가 작성한 스케줄에 의해 순서대로 자기 차례에 실행되는 구조다.
...
- Port Forwarding : 외부에서 내부의 컴퓨터중 하나로 접속할수있게 하는 기술
	공인ip:포트번호 -> 사설ip:포트번호
	공유기의 포트포워드 설정에서 관리
  iptime은 192.168.0.1    admin/admin
  sk broadband는 192.168.45.1  아이디 admin/공유기뒤 유선맥주소 뒤에 진한 6글자_admin
...
### 세션
> 티켓을 반으로 잘라, 반쪽은 사용자의 브라우저, 반쪽은 서버에 저장(램, 하드, db 등)
- 요청마다 이 표딱지를 실어보낸다. Session Id를 검사.
- 서버의 메모리 부족. 서버가 꺼지거나 하면 서버에 저장된 반쪽 다 날라감.
  즉 서버 재시작하면 세션 다 끊김.
- 대규모라서 여러대의 서버를 쓴다면 세션 유지 더 어렵지.
- 장점 : 구현이 쉽다. 특정상황에서 세션을 직접 핸들링 할 수 있어서 상황대처에 용이


### JWT : JSON Web Token
> 메모리, 과부하, 확장, 개발 및 유지보수 비용 등 세션 인증방식의 문제점을 해결
> **무상태(Stateless)**가 가장 큰 특징
> 대칭키방식으로 암호화된 신분증. 역시 토큰을 발급하는데 찢어서 주지않고 그냥 준다.
  서버가 뭔가 기억하지 않는다는 얘기다.
- 헤더, 페이로드, 서명 정보로    인코딩된 3가지의 데이터를 이어붙인 토큰을 발급한다.
- base64로 디코딩 할 수 있음. 조작을 막기위해 1,3번 정보가 있는거다.
- 단점
  세션은 기억하는 대상의 상태들을 언제든 제어할수 있다. 예를들어 강제 로그아웃, pc, 폰 로그인 등 동시로그인 등
  그러나 JWT는 무상태고 만료주기가 있기 때문에 위같은 제어를 할수 없다. 즉, 한번 탈취당하면 심각한 상황.
  그래서 만료기간을 짧게 두는 방식으로 보완하며 사용한다.
- .을 기준으로 좌측부터 Header, Payload, Signature를 의미하는 문자열의 조합
  이런 인증에 필요한 정보가 모두 담겨있기 때문에 Stateless가 가능한 것.
  - Header : JWT에서 사용할 토큰의 타입과 해시 알고리즘의 종류를 규정
  - Payload : 서버에서 첨부한 사용자 권한 정보와 데이터
  - Signature : Header, Payload를 Base64 세이프 인코딩을 한 후 해시함수를 적용하고 개인키로 서명한 전자서명이 담겨있음.
...
#### HTTPS
HTTP + Over Secure Socket Layer(SSL)  보안이 강화된 HTTP
그냥 HTTP는 클라이언트가 서버로 데이터를 보낼때 암호화없이 보내기 때문에 악의적인 감청이나 데이터의 변조등이 일어날 수 있다.
HTTPS와 SSL은 완전히 같은 의미가 아니다.  웹이 인터넷 위에서 동작하는 하나의 서비스인것처럼 HTTPS도 SSL프로토콜 위에서 돌아가는 프로토콜이다. 즉, HTTP가 SSL을 이용하는것이 HTTPS다.
HTTPS가 아니면 브라우저는 경고를 띄워주고, 검색엔진에서도 도메인을 평가할때 중요한 순위다.

#### SSL
[생활코딩](https://opentutorials.org/course/228/4894)
SSL에 대해 전반적으로 이해하려면 상당히 많은 개념들이 유기적으로 상호작용해야하기 때문에 여러번 읽어야 한다.
![SSL프로토콜](../이미지/SSL프로토콜.png)
SSL은 네스케이프에서 발명했는데, 버전3부터 표준화기구 IETF가 관리하게 되면서 이름을 바꿨는데 그것이 TLS다.
SSL 인증서는 클라이언트와 서버간의 통신을 제3자가 보증해주는 인증문서다.
클라이언트가 서버에 접속하면 서버가 제일먼저 하는일이 이 인증서 정보를 전달하는 것이다.
그럼 클라이언트는 이 인증서 정보가 신뢰할 수 있는것인지를 검증하고 나서야 다음절차를 수행하게 된다.
CA(Certificate Authority)는 신뢰성이 엄격하게 공인된 기업만이 참여할 수 있다. 
인증서에 대한 보증을 서준다. 클라이언트가 접속한 서버놈이  클라이언트가 접속을 원했던 서버가 맞는지를 보장하는 역할을 한다.
Symantec, Comodo, GoDaddy, GlobalSign 등의 업체들이 있다.
국내의 웹호스팅업체들이 CA를 대행해서 인증서를 판매하니 영어나 행정절차가 부담된다면 그것을 이용하면 된다.
개발이나 사적인 목적을 위해 ssl의 암호화 기능을 사용할때는 직접 CA의 역할을 할수 있는데 그러면 브라우저가 경고를 출력한다.
공인된 CA를 사용하면 초록색 자물쇠가 뜬다.
브라우저는 공인된 CA의 리스트와 각 CA의 공개키를 미리 파악하고 있어야 한다.
해당 CA의 비공개키를 가지고있는 CA는 해당 CA밖에 없기 때문에 CA의 검토를 통과했다는 말은 해당 서비스가 신뢰할 수 있다는것을 의미한다.
...
- 서버에 인증서만 만들어둔다고 ssl이 그냥 적용되지 않는다. 웹서버에게 설정을 알려줘야한다. 
  아파치라면 ssl.conf 열어서 아래영역 찾아서 자기꺼에 맞는 값으로 변경
  https://blogger.pe.kr/571
  #   Server Certificate:
...
http는 tcp/80
https는 tcp/443
https적용이 되있더라도 브라우저에서 http로 요청을 하면 암호화되지 않은 통신을 하게 된다.
그렇기 대문에 http접속이 오면 https로 리다이렉션 되게 해야한다.
httpd.conf파일을 수정하는것이 좋다.
<VirtualHost *:80>
  ServerName 도메인주소
  Redirect permanent / https://도메인주소/
</VirtualHost>

톰캣이 아니라 정적파일 아파치를 썼다거나 서버를 내마음대로 못건들경우는 아래방법이 좋다.
(**중요**)
.htaccess는 하이퍼텍스트 액세스(hypertext access)의 줄인 말로 아파치 서버 (Apache) 등 여러 웹서버가 지원하는 디렉터리 수준의 설정 파일입니다.
원래 목적은 디렉토리 접근을 제어하는 것이었지만 리다이렉트 등등 다양한 방법으로 활용하실 수 있어요.
각 웹서버에서 도메인이 연결된 홈 디렉토리의 root디렉토리에 위치하면된다. 없으면 만들면된다.

#Force SSL
RewriteEngine On
RewriteCond %{SERVER_PORT} 80
RewriteRule ^(.*)$ https://www.도메인주소/$1 [R,L]

//www가 있고 없고도 중요하니까 잘보고 작동
...
### TCP/IP
웹서버에 소켓이 열려있다. 소켓은 TCP/IP를 추상화한것이고 본질은 파일이다.
파일의 권한은 RWX(읽기 쓰기 실행)가 있는데 소켓입장에서는 실행이라는게 없고,   읽는다는건 Receive, 쓴다는건 Send  

## 소켓프로그래밍
- 네이버 실시간 검색어같은거
ajax로 해당 부분만 서버에 조회하는것 보다 나은것이
서버측에서 데이터가 변경됐을때 클라이언트쪽으로 렌더링 (SSE)
> 실시간 양방향 통신을 지원. 한번 연결이 수립되면 클라이언트와 서버 모두 자유롭게 데이터를 보낼 수 있다.
채팅같은 서비스에 적합하다.
> 네트워크로 통신하는 모든 프로그램이 결국 소켓을 활용한것은 맞다. 직접적으로 로우레벨에서 소켓을 쓰냐의 차이다.
- 소켓이란? 프로세스간의 통신에 사용되는 양쪽 끝단을 말한다.
- TCP/IP 프로토콜은 이기종 시스템간의 통신을 위한 표준 프로토콜
  TCP소켓 프로그래밍은 클라이언트와 서버간의 일대일 통신
- 서버소켓은 소켓간의 '연결'만 처리하고 실제 데이터는 소켓들끼리 서로 주고받는다. 입출력스트림을 통해서.
- 일반적으로 HTTP에서는 ws, HTTPS에서는 wss로 통신해야 한다.
- HTTP통신은 지속적으로 데이터를 요청하는 폴링방식을 사용해야 하지만, 웹소켓은 그럴 필요가 없다. 그래서 더 낮은 부하를 사용하여 클라이언트와 서버간의 실시간 통신이 가능하다.
  
  //내가했던거 기억을 되살려보자.
  1. 약속된 서버의 IP와 포트로 연결 요청            //입장되는지 문의
  2. 서버가 나인지를 확인하고 새로 연결할 포트를 줌  //문지기가 입장 대기표줌
  3. 그 포트로 새로 소켓생성하고 연결요청함         //저자리에 앉아도되나요?
  4. 서버가 서버에 새로소켓생성하고 연결해줌        //네, 안내해드릴게요.
  5. 계속 리스닝하고있으면서 전문의 헤더에 특정 코드가 있는지 확인
  6. 내가 처리해야될 코드가 인지되면 분기처리
  7. 로그로 남기고 처리됐다고 알려줌
  8. DB에 기록
  9. 리스닝의 반복
  10. 서버가 형식이 이상한 전문을 보냈거나, 안보냈거나, 내가 이상할때 등 여러 예외처리
    손실된 경우 재전송 
    서버가 보내는게 없을때도 연결 확인 체크 
  11. 멀티스레드, 배치파일로 스케쥴러에서 자동실행, 안꺼지는 데몬 프로젝트
    데몬으로 돌렸기 때문에 도중에 사용자가 명령을 내릴수 없음.

## 웹소켓
[스프링부트 웹소켓과 채팅](https://dev-gorany.tistory.com/212)
- WebSocket
  : 기존의 단방향 HTTP 프로토콜과 호환되어 양방향 통신을 제공하기 위해 개발된 프로토콜.
  내가 서버에게 요청을 보내지 않아도 서버가 나에게 정보를 주어야 할때 사용.
  -  원하는 정보에 구독(subscribe)을 하고 구독한 토픽에 대해 메세지를 발행하면 해당 토픽을 구독하고 있는 모든사용자에게 메세지를 보내주는 방식이 바로 웹소켓이다.
...
- 접속과정
  1. TCP/IP 접속 요청 (클라가)
  2. TCP/IP 접속 수락 (서버가)
  3. 웹소켓 열기 핸드쉐이크 요청 (클라가)
  4. 웹소켓 열기 핸드쉐이크 수락 (서버가)
  5. 웹소켓 데이터 송수신 (서로)
- 텍스트기반의 채팅을 구현하려면 TextWebSocketHandler를 상속받아서 작성
- payload
  데이터를 보낼때 Header, Meta데이터, 에러체크비트 등을 함께 보내어 전송 효율과 안성성을 높인다.
  이런 송장번호, 박스, 스티커, 테이프 등은 빼고 택배 물건 자체만을 페이로드라고 한다.
  보통의 JSON에서 data가 페이로드다.
- Message 인터페이스는 헤더와 페이로드로 구성된다.
- 핸들러
  @EnableWebSocket 어노테이션을 사용해 WebSocket을 활성화
  EndPoint는 /chat으로 설정
  도메인이 다른 서버에서도 접속 가능하도록 CORS : setAllowedOrigins("*"); 를 추가
  이제 클라이언트측에서는 ws://localhost:8080/chat  로 접속할 수 있다.
    ```java
    @Configuration
    ...
    ```
  - Endpoint
    API가 서버에서 자원에 접근할 수 있도록 하는 URL을 말한다.

#### STOMP
Simple Text Oriented Messaging Protocol
웹소켓 서버가 2대 이상이라면 메모리 기반으로 관리하는 세션정보를 서로 알아야 한다.
그것을 하기위해 다양한 방법이 있다.
1. STOMP
2. Redis Pub/Sub
3. 어떤 세션이 어느 서버에 저장하고 있는지 별도의 저장소에 저장

STOMP를 쓰면 WebSocketHandler를 직접 구현할 필요가 없다.
- @MessagingMapping 등의 어노테이션을 사용하여 메세지 발생시 엔드포인트를 별도로 분리해서 관리할 수 있다.
- @EnableWebSocketMessageBroker
를 써야 STOMP를 사용할 수 있다.
registry.addEndpoint("/ws/chatroom").setAllowedOriginPatterns("*").withSockJS(); 엔드포인트
registry.enableSimpleBroker("/queue", "/topic"); 구독경로
registry.setApplicationDestinationPrefixes("/app"); 발행경로
- brokerChannel 
서버내부에서 사용하는 채널이며, 이를통해 SimpleAnnotationMethod는 SimpleBroker의 존재를 직접 알지 못해도 메세지를 전달할 수 있다

#### 암호화
암호화를 할때 사용하는 일종의 비밀번호를 키라고 한다. 키를 모르면 복화화를 할 수 없다.

##### 비밀키  #### 대칭키  Symmetric Key
> 동일한 키로 암호화와 복호화를 같이 하는 방식.
> 하나의 비밀키를 클라이언트(송신자)와 서버(수신자)가 같이 사용
예를들어 1234로 암호화를 했으면 복호화를 할때도 1234 사용
단점 : 암호를 주고받는 사람들사이에 대칭키를 전달하는 것이 어렵다.
      한쪽이라도 키가 유출되면 그 비밀키는 안전보장 안됨.

##### 공개키 #### 비대칭키 Asymmetric Key
대칭키 방식을 해결하고자 나온 방식.
타인에게 제공하는 공개키(public key)와 자기만 가지고 있는 비공개키(private key)가있다.
공개키를 가지고 있는 타인은 공개키를 사용하여 암호화하고, 나는 비공개키로 복호화한다.
공개키로는 암호화는 할수 있지만 복호화는 할 수 없기 때문에 더 안전하다.
공개키가 데이터를 제공한 사람의 '신원을 보증'하는 역할을 한다. 이것을 전자서명이라고 한다.
- 대표 : RSA, SSL
- 단점 : 암호화속도 느리다, 암호화할수있는 평문의 길이 제한


### 서버리스 ### Serverless
> 서버가 없다가 이름이지만 내가 직접 서버를 관리하지 않는것을 말한다.
- 장점
  - 24시간 돌아가고있는것이 아니라 요청이 들어오면 깨웠다 없으면 잤다가 반복. 이것이 혁명
  - API호출만큼만 돈을 지불하면 된다.
  - 트래픽이 많아져도 퍼포먼스에 영향이 없다. API를 일시적으로 더 많은 서버에 만드는 방식으로 작동해주기 때문
- 단점
  - cold start.    자고있기 때문에 깨우는데 시간이 걸린다.
  - 서버제공자에 너무 의존하게 된다. 서버에 대한 통제를 잃음. 서버리스로 구축했다가 다른곳으로 옮기기가 쉽지 않다. 
- 언제 사용하면 좋은가
  - 사이드 프로젝트
  - 최대한 빠른 프로토타입
- 제품
...
- [웹소켓 채팅](https://tarotaro90.com/ko/%EC%9B%B9-%EC%86%8C%EC%BC%93%EC%9D%84-%EC%9D%B4%EC%9A%A9%ED%95%9C-%EC%B1%84%ED%8C%85-%EA%B5%AC%ED%98%84/)

### Redis
Key-Value형태의 비관계형 데이터베이스(NoSQL)
속도가 매우 빠른 장점이 있어 캐시나 세션 저장소로 많이 사용한다.
- 원격에 존재.  프로세스로 존재.  '메모리기반'.  '키-값 구조'
- 키-값 구조이기 때문에 쿼리없이도 데이터를 간단히 가져올 수 있다.
- 지원하는 자료구조 : String, Set, Sorted Set, Hash, List
- 대표적특징 : 싱글쓰레드

- 작은 서비스는 WEB-WAS-DB의 전형적인 3티어 구조를 취하는것이 보통이지만 대용량트래픽이 발생하면 DB에 무리가 가기 시작한다. 
그래서 캐시서버 도입을 적극적으로 검토하게 된다. 캐시는 보통 '메모리'를 사용하기 때문에 속도가 상당히 빠르다.
캐시서버로 널리 쓰이고 있는 redis에 대해 알아보자.


### OAuth
Open standard for Authorization
**인가(권한 허가)를 위한 개방형 표준 프로토콜.   인증을 위한 프로토콜 아니다.**
  //다만 인가를 받기 위해서는 인증이 선행되어야 하기 때문에 보통은 같이 사용한다.
Third-Party 프로그램에게 리소스 소유자를 대신하여 리소스 서버에서 제공하는 자원에 대한 접근 권한을 위임하는 방식을 제공한다.
쉽게말해서 우리고객이 다른서비스의 회원정보를 우리서비스에 안전하게 건네주기 위한 방법
예 : sns로그인, google로그인, github로그인, apple로그인

- OAuth의 역사
1. OAuth등장 이전의 기본적인 ID/PW는 보안상 취약점이 컸고, 안전한 표준인증방식이 없었다.
2. OAuth 1.0 : API로 접근권한을 관리하는 구굴의 AuthSub나 야후의 BBAuth등을 참고하여 발표
  비공식 논의체에 의해 만들어졌으나 OAuth에 대한 커뮤니티가 성장하면서 지속적 발전
3. OAuth 2.0o: 1.0의 구조적인 문제점을 해결하기 위해 등장한 WRAP을 기반으로 개발된 프레임워크
  탈브라우저, HTTPS필수 사용, 전자서명 단순화, 엑세스토큰의 생명주기감소
  가장큰변화는 요청을 처리하는 서버와 **사용자 인증을 위한 서버의 역할을 명확히 구분**
  1.0과 2.0은 많이 달라서 상호호환되지 않는다.
  IETF OAuth 워킹 그룹이 주축이 되어 만들어졌다.

- **Access Token**
고객이 정보를 넘겨주는 것을 동의했다는 징표
그럼 억세스 토큰을 어떻게 네이버로부터 받나? 당연히 네이버가 건네줘야 한다. 
그 고객이 네이버에 로그인을 하면 네이버가 그 고객은 자기들의 고객임을 인증하고 토큰을 넘겨준다.
네이버가 리다이렉트할 URI를 쿼리스트링으로 전달해주고
XSS공격이나 피싱사이트를 이용하는것을 막기위해서 해당서비스에 등록절차를 밟아야 한다.
즉, 우리서비스에서 네이버로그인기능을 사용하려면 사전에 네이버에 등록을 하고 승인을 받아야 한다.

#### OAuth 2.0
  매커니즘이 하나로 통일되지 않아 4가지 타입으로 발표됨
  - 구성요소
    - 자원소유자
    - 클라이언트
    - 인가서버
...
### NoSQL
정해진 스키마가 없다.
장점 : 데이터 구조 변화가 자유롭고 분산이 용이
단점 : 데이터 중복이 발생하거나 변경시에 연산이 오래걸림

### 병렬 프로그래밍
- 큰 문제를 작게 나누어 동시에 해결할때 사용
- 멀티프로세싱, 멀티쓰레딩의 차이 설명,
- 데드락에 유의
  뮤텍스 : 한스레드, 프로세스에 의해 소유될 수 있는 key 기반
  세마포어 : 현재 공유자원에 접근할 수 있는 스레드, 프로세스의 수를 나타내는 값에 기반

# CI/CD
> CI(Continuous Integration) 지속적 통합
  개발할때 변경한 코드를 지속적으로 통합시킴으로써 품질 유지
> CD(Continuous Deploy 또는 Delivery) 지속적 배포
  변경된 내용에 대한 자동 버그테스트, 자동 업로드, 병합후 운영서버까지 자동배포
- 배경
  개발이 다 끝나고 코드품질 관리를 하면 충돌과 오류의 크기가 커지기 때문에 개발의 통합과 배포를 보다 짧은 주기로 하여 고객에게 제공하는 방법.
  개발자와 사용자 사이의 격차를 줄이는 것
- 툴
  Hudson, Jenkins, Travis CI, Circle CI, Github Action, Bamboo, Gitlab
- 예)
...
# jenkins
> CI/CD 도구중 가장 대표. 오픈소스
> 허드슨이라는 이름으로 알려졌지만 2010년 오라클과의 상표권 문제로 Jenkins로 이름 변경
- 개요
  - 자바 런타임 환경에서 동작
...
- 기능과 장점
  ...
  - 결합 테스트 환경에 대한 배포작업
  - 형상관리툴과의 연동
  - 500여가지가 넘는 플러그인을 간단히 인스톨
    

- 단점
  - 기존의 배포서버와 분리된 젠킨스 전용서버가 필요
  - 많은 시간이 소요 (세팅이 어렵다)
  - github이벤트가 기준이 아니고 빌드(서버에 올릴 수 있는 상태로 만드는 것)를 중심
  - 도커환경이 필요함
  - 공유 안됨
  - 무거움
...
- 버전
  Jenkins2는 비약적인 발전
  높은 수준의 유연성과 관리가 용이한 파이프라인을 설계, 구현, 실행 가능

- 파이프라인
  - CI/CD 파이프라인을 젠킨스에 구현하기 위한 일련의 플러그인들의 집합이자 구성
  - 파이프라인 DSL로 작성됨
  - 젠킨스가 동작한다는것은 여러 플러그인들이 파이프라인을 통해 흘러가는 과정이라고 할 수 있음
  - 간단한 단일작업은 파이프라인보다 Freestyle로 웹화면에서 메뉴눌러서 정의
  - 파이프라인은 4개의 섹션으로 구성됨
    - Agent section : 어떤 시종에게 일을 시킬것인지 결정
    - Post section : 스테이지가 끝난 이후 결과에 따른 후속조치
      - success, failure, always, cleanup
    - State section : 처리할 일들의 묶음(카테고리)
    - Steps section : 한스테이지 안에서의 단계. 플러그인들을 깔면 사용할수있는 스텝들이 생겨남
  - pipeline => agent => stage=> step 순으로 작업이 세분화

- Item
  새롭게 정의할 작업을 의미. 프로젝트.

- 젠킨스 마스터(서버)
마스터노드는 젠킨스 파이프라인으로 정의된 모든 흐름을 관장하는 컨트롤 서버. 젠킨스 슬레이브들에게 각각의 할일을 분배한다.
...
1. JRE위에 젠킨스 설치
  - 윈도우
    ...
    1. 운영체제 선택. 윈도우라면 .msi
    1. 아이디 비번 입력후 Test Credentials
    //this account either does not have the privilege to logon as a service 에러나면 Windows 관리 도구 - 로컬 보안 정책 - 로컬 정책- 사용자 권한 할당 - 서비스로 로그온 - 사용자 또는 그룹 추가 에서 사용자 추가
    1. port 번호와 jdk지정
    1. 웹으로 접속후 계정생성
  ...
  - 리눅스
    ...
    젠킨스는 내부적으로 톰캣을 서버로 사용하므로 톰캣 기본포트인 8080을 사용하며 시작된다. 젠킨스의 기본 설정파일은 /etc/default/jenkins다.
    1. jenkins 실행
...
2. 젠킨스 접속
  http://서버ip:포트번호
  초기비밀번호 입력창이 뜨면 sudo cat /var/lib/jenkins/secrets/initialAdminPassword 쳐서 나온 비밀번호 입력

3. 추천플러그인 설치
Install suggested plugins버튼
//Git Plugin, Pipeline, Credential Plugin 등
Build Pipeline Plugin : Job들간에 관계 제공
Parameterized Trigger Plugin : Build Pipeline Plugin에 종속성. 트리거에 파라미터를 보낼 수 있게 해줌
dashboard-view : 연관관계가 있는 Job들간에 대쉬보드 제공
Parameterized Build : 파라미터를 사용하여 빌드할 수 있는 환경 제공
...
5버전(Jupiter)이 나왔고 많이 바겼다.
JUNIT은 테스트 프레임워크. 기능테스트 중심으로 생각하는게 좋다.
...
- 문법
1. @Test : 해당 메소드가 단위 테스트임을 명시하는 어노테이션. JUNIT4부터 생김
...
<팁>
- AOP에서 배우는 @Before, @After도 잘 활용하면 좋다.
- 어플리케이션 컨택스트에서 테스트를 분리하는 것이 좋다.
...
- 스프링에서 web.xml에 설정파일들 메타데이터 넣은것들은 junit을 통한 테스트에서 작동하지 않는다. 예를들어서 log4j
log4j:WARN No appenders could be found for logger (org.springframework.test.context.junit4.SpringJUnit4ClassRunner).
log4j:WARN Please initialize the log4j system properly.
log4j:WARN See http://logging.apache.org/log4j/1.2/faq.html#noconfig for more info.
이런 에러들이 뜨고 테스트메소드가 실행안된다.
web.xml을 읽어서 환경을 구성해줄 컨테이너가 없기 때문이다.
이럴경우를 대비해서 스프링은 ApplicationContext에서 직접 설정파일의 위치를 받을수있는 클래스가 만들어져있다.
...
- 테스트코드는 유용한 부산물이지 중요한건 사고방식
  - 결정에 대한 피드백을 빨리 받는다.
  - 피드백은 자동화된 컴퓨터로 부터 받는다.
- TDD는 언제 하면 좋은가?
  - 요구사항의 변경가능성이 높거나 개발과정에 코드가 많이 변경될 경우
  - 유지보수를 내가 아닌 다른사람이 해야할 경우
  - 불확실한 상황일수록 TDD로 개발할때 빠르다.

# 빌더패턴
생성자가 아니라 builder()함수를 통해 객체를 생성할수 있는 빌더를 얻고,
거기에 세팅하고자 하는값을 전달해 마지막에 build()를 통해 빌더를 작동시켜 객체를 생성하는 방식
<사용이유>
- 생성자를 쓸경우 파라미터가 많을 경우 가독성이 좋지않다.
- 어떤 값을 먼저 설정하든 상관없다.

<구현방법>
@Builder 어노테이션 달고 메서드 체이닝
롬복에서 제공함
...
### Heroku
간단하게 CI/CD 하고 싶을때 쓰면 좋다.

#### serialVersionUID
직렬화할때 사용
직접할당하지 않고 컴파일러에게 맡길 경우 아래와 같은 경우 문제가 생긴다.
- 저장하려는 쪽의 컴파일러와 불러오는 쪽의 컴파일러가 다를 경우
- 저장하는 시기의 클래스 내용과 불러오는 시기의 클래스 내용이 다를경우
제대로 객체를 불러올 수 없을것이다.
그렇기 때문에 serialVersionUID를 지정하여 사용.


## SOP
Same Origin Policy
한 origin으로 부터 로드된 document 또는 script가 다른 origin의 리소스와 상호작용 할 수 있는 방법을 제한하는 중요한 보안 메커니즘
- 쉽게 말해서 document가 다른 리소스와 상호작용을 할때 리소스의 원천이 document의 원천과 다른경우(Cross Origin일 경우) 제한을 두겠다.
- Cross Origin간에 데이터 교환이 필요할때는 window.postMessage API사용
- 오리진확인 : 브라우저 콘솔에서 document.location.origin 쳐보면 확인가능
- 오리진 판단 규칙 : scheme, host, port 3가지만 본다.     예를들어 https://www.dokydoky.com:443/path/page.html 주소라면 https://www.dokydoky.com:443까지
- CSRF공격(//write는 가능하기 때문)

# 노하우

### 엑셀과 연동한 노하우
(**중요**)엑셀에서 update쳐달라는거 한줄 update문 문자열 수식으로 만들고 복사해서 여러줄 update 한번에 실행
&은 함수와 함수를 이어주는 기능
"는 문자열을 쓸때 사용
'는 문자열이 아니다.
="select '" & A2 & "' as NCSCD, " & C2 & " as 취업률, " & D2 & " as 일반 union all"
쉽게말해서 '를 문자그대로 작은따옴표로서 텍스트를 작성하려고 하고 문자열은 다 쌍따옴표 안에 넣어주면되고 값은 마우스로 찍어서 쓰면된다.
//작은따옴표를 수식에서 넣으려면 ="'"&A1&"'"

### 데이터조회
- 운영서버에서는 데이터확인용으로 조건절 없이 조회할때 항상 TOP을 걸어주기
- 서브쿼리는 최대한 지양.  대신 JOIN을 활용. JOIN도 최대한 INNSER JOIN.    인덱스가 있는 애들끼리 조건