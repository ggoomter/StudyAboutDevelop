...
# 서블릿 Servlet
  - Server Program + let(웹 서버에서 실행되는 작은 프로그램 단위)   또는 Server + Applet
  ...
  - javax.servlet.http.HttpServlet을 상속받아 서블릿 클래스를 구현한다.
  ...
  - [동작방식]
    ...
    1. 해당 서블릿에서 service메소드를 호출한 후 클리아언트의 GET, POST여부에 따라 doGet() 또는 doPost()를 호출합니다.
    ...
    결론적으로 서블릿을 관리해주는 서블릿 컨테이너가 있어야 한다.
...
- 특징
	...
	- 공통 page부분은 fragment로, layout을 조립할수 있다.
		예)
...
1. html파일을 만들고 <html xmlns:th="http://www.thymeleaf.org"> 로 수정한다.
...
#### th:속성   
  ...
  ★★ th:fragment="프로그먼트명" 공통으로 사용되는 페이지를 정의하는 부분을 fragment로 선언한다.
  ★★ th:replace="프래그먼트경로 :: 프로먼트명"
...
10. 명사에는 단수형보다는 복수형을 사용한다. 특히 컬렉션은
