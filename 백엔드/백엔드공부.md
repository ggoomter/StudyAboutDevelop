...
- DAO : Data Transfer Object.  데이터 전송 객체
  계층간 데이터 교환을 위한 자바빈즈라고 생각하면 된다.
...
# MVC
  ...
  - 왜쪼개느냐? 입법, 사법, 행정으로 나누듯이 규모가 커지게되면 나눠야 관리가 편하다. 이 기초골격 자체를 미리 짜놓고 제공하는것.
  ...
  - 장점
    ...
    ## Mapper
      ...
      - 사용법
        ...
        - # 와 $ 의 차이
          - preparedStatment와 Statement의 차이점이라고 보면 된다. 동적과 정적의 차이. 대부분 #을 쓴다.
          #{변수}를 사용하면 DTO의 getter메소드를 사용하여 해당하는 변수값을 가져온다. sql injection을 예방할수있어서 보안측면에서 좋다.
                    ${변수}를 사용하면 파라미터가 바로 출력된다. 문자열에 자동으로 ''를 감싸주지 않는다. 재사용이 불가능하고 새로운 쿼리로 인식한다.

    - null처리
    db연결맺는 설정파일로 가서 jdbcTypeForNull 설정
    예)
...
#### interceptor
- 컨트롤러에 들어오는 요청을 대한 컨트롤러가 작업하기 전/후로 가로챈다.
...
우리가 www.icann.org 를 치면 도메인서버가 192.0.43.7:80(매핑된 ip주소:브라우저를 통해요청했으니80포트) 으로 접속해서 그 서버가 반환하는 첫페이지를 다시 우리에게 보여주는 것이다.
이름을 붙인다는 것은 위대한일
...
---
### 세션, 토큰, jwt
로그인상태를 유지시키는 목적으로 주로 쓰이는 것
인증 : 계정 확인
인가(어서와이제이션) : 인증받은 사람의 권한에 대한 허가

세션 : 반쪽은 사용자의 브라우저, 반쪽은 서버에 저장(램, 하드, db 등)
...
본사가 아니라 체인점 지점으로 가는 매커니즘이다. CDN업체에는 상당수가 캐싱되어있다.
