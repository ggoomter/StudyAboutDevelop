...
- PRIMARY Key(기본키)  = unique && not null. 한 행을 식별하기위한 기준
    - PK는 유니크와 낫널이의 엔드조건이다. (O)    유니크와 낫널이면 PK다.(X)
- FOREIGN Key(외래키)  = 참조하는 테이블(부모)에 존재하는 값들만 사용해야 함
- CHECK   = 조건에 부합하는 데이터만 입력 가능
---

# Entity (엔터티)
- 엔터티는 **집합**이다. 사전적의미는 독립체. 쉽게말해서 업무에 쓰이는 데이터를 용도별로 분류한 그룹. 나중에 테이블로 변경된다.
  각각의 엔터티는 속성을 갖게되는데 나중에 테이블의 Column이 된다.
  인스턴스들이 만들어지면 테이블의 Row가 된다.

  (중요)메인엔터티는 **개체**든 **행위**든 하나의 순수한 집합이여야 한다.
  집합은 정의 가능한 사물 또는 개념을 의미한다. 그러나 집합이라고 다 엔터티는 아니다.
  예를 들어 '납입자'는   납입이라는 엔터티와 사람이라는 엔터티가 연결된 관계다.  '청구서' 이런거 마찬가지겠지.

- ### 엔터티 이름짓기
  - 갖고있는 데이터가 무엇인지 명확하게 표현하며 다른 엔터티와 의미상 중복없게, 업무에서 실제로 쓰이는 용어 사용, 한글은 약어를 사용하지 않고 영문은 대문자로 표기,  단수명사로 표현하고 띄어쓰기는 하지 않음
  - 엔터티 이름 짓기 매우 중요하다. 누가보더라도 어떤 집합인지 명확하게 느껴지도록.
  - 엔터티의 이름이 잘 안뽑힌다면 집합에 대한 정의가 잘 안됐기 때문이다.
- ### 순서
  **후보 엔터티** 뽑기 -> **키 엔터티**(부모가 없이 자기 스스로 탄생가능) -> **메인엔터티**(업무의 중심) -> **자손엔터티**
- 프로그램과 화면에서 가장 많이 다루게될것은 자손엔터티가 될것이다. 근간이 되는 데이터는 잘 변하지 않는다.
...
- ### 발생시점에 따른 분류
  - 기본 엔터티 : 독립적으로 생성되어 자신만의 주 식별자를 가지며 다른 엔터티의 부모 역할을 함
  - 중심 엔터티 : 기분 엔터티로부터 파생되어 많은 데이터를 갖게되며 행위 엔터티를 생성함
  - 행위 엔터티 : 두개이상의 부모 엔터티로부터 파생되고 보통 설계 초기단계보다는 상세 설계단계에서 많이 도출됨

- ### 엔터티인지 검증(판별 규칙)
1. 동질성을 가진 집합인지(정의한 집합 테두리에 포함되는지)
1. 독립성을 가진 집합인지(부분집합이 아니여야하고 교집합이 없어야 한다.)  엔터티끼리 교집합이 생긴다면 문제가 생긴거다. 예) 가입자, 납입자
1. 가로*세로로 면적이 되어야한다.
가로(속성)가 2개이상이고, 세로(데이터)가 2개 이상이어야 한다.
2개 이상의 인스턴스를 가지고 있어야한다. 대표라는 엔터티가 있는데 현재 대표가 1명이고 앞으로도 쭉 1명일 예정이라면 굳이 엔터티로 만들 필요가 없다.
1. 행위엔터티와 개체 엔터티를 합친 엔터티는 분리해야된다.
1. 최소한 하나이상의 다른 엔터티와의 관계를 가지는지
1. 식별자가 있어야한다.

## 속성
 - 속성은 특성에 따라 기본속성, 설계속성, 파생속성으로 분류할 수 있으며, 데이터조회시 빠른 성능을 보장하기 위해 본래의 속성값을 계산하여 따로 저장할 수 있도록 만든 속성을 파생속성이라 한다. 반드시 데이터의 정합성이 고려되어야 한다.
 - 도메인 : 속성이 가질수 있는 값의 범위

## 용어사전
하나의 데이터베이스에서 같은 의미를 가진 데이터가 엔터티마다 다른 명으로 정의된다면 혼란스럽기 때문에 설계 시 용어사전을 두고 공통된 룰로 적용하는 것이 바람직하다.

---
# 기본키
- **한 테이블의 유일함을 결정짓는 속성**
- 한 테이블의 기본키는 하나의 컬럼에만 지정할수 있다? 맞을까? 틀릴까?
- "한 테이블은 오직 하나의 기본키를 가질 수 있다." 이건 정확한 정의다.
...
---
# 모델링의 순서
- 요구사항 분석 -> 개념적 모델링(주제별 핵심엔터티, 관계) -> 논리적 모델링(정규화. 모델링의 핵심) -> 물리적 모델(실제컬럼명, 데이터타입, 길이, 실제저장공간, 분산, 인덱스 설계, 성능최적화) -> DB스키마 구현후 스크립트 파일로 보관 -> 테스트데이터 입력
...
## ER모델
  ...
  표기법
  6개가 있지만 대표적으로 바커표기법가 IE/Crow's Foot표기법이있다.
  김이사님이 쓰는건 IE/Crow's Foot표기법.
  바커표기법은 한쪽에서 나가고, 다른쪽에서 나간걸 합친것. 오라클에서 사용되는 모델
  ![바커표기법 IE표기법](./이미지/바커와IE표기법.png)


## 관계 [관계링크](http://wiki.gurubee.net/pages/viewpage.action?pageId=26743462)
  IE표기법을 기준으로 설명.
  ...
  관계명은 2개인데 각 엔터티의 관점에서 관계명을 하나씩 가지기 때문이다.
...
- M:M관계는 모델링이 진행되는 과정의 덜풀린것이다. M:M은 최종물리모델에서는 구현될 수 없기 때문에 모델링이 끝나면 M:M관계 없어야한다. 다대다 관계를 실제로 구현하기 위해선 각 테이블의 PK를 FK로 참조하고 있는 **연결테이블**(**관계테이블**, **관계해소테이블**)을 사용해야 한다.
        ...
        M:M을 관계에서 관계엔터티로 언제 풀거냐 하는걸 잘 결정해야한다.
    )
...
## Optionality
연관되는 엔터티의 값이 반드시 존재해야하는지(Mandatory), 존재하지 않아도 되는지(Optional)를 의미
...
상위엔터티쪽의 옵셔널리티가 필수가 되는것이 좋다.
아래와같이 바꾸면 통장과 고객, 고객에게 발급된 통장이 되어 엔터티의 성격이 명확해진다.
...
부자관계가 아니다. 부분집합관계.   예) 고객테이블의 개인고객, 법인고객.  별도의 엔터티가 생성되지 않는다.
->논리적 모델링에서 테이블을 설계할때 각각의 테이블로 설계하는 방법이 있고 슈퍼타입/서브타입으로 설계하는 방법이 있다.
엔터티를 일반화 하면(공통점을 묶으면) 슈퍼타입이 생긴다. 상세화하면 서브타입이 생긴다. 서브타입은 슈퍼타입의 부분집합이다. 즉 중복속성을 제거하기 위해 슈퍼타입으로 만든다.
...
- #### TYPE2( Rollup )
  **슈퍼타입**으로 통합(모델을 통합하는것이므로 역정규화)
      ...
      장점 : 조회SQL이 단순해지고 성능이 좋아질때가 많다.
            ...
            복잡한 관계가 없어지고 엔터티의 수가 감소해 ERD관리가 용이하다.
      단점 : 크기 증가, NULL을 가진 속성 많아짐, 업무가 변경되면 영향도 증가, 엔터티의 정체성 희석
- #### TYPE3( identity )
  슈퍼타입, 서브타입 **혼합**
      예
      ...
      장점 : 핵심 조회 요건의 성능이 좋아질때가 많다
           ...
           추가업무에 대한 영향도 줄어든다. 변경요건이 각 엔터티에 분산되기 때문.
      ...
      유의 : 이것이 정상적인 설계패턴.
        ...
        슈퍼타입과 서브타입 각각 별도의 관계를 맺은 엔터티들이 존재하는 경우
...
- 예 : 학생, 교사 엔터티만 있었는데, 행정직원 엔터티, 청소용역 엔터티 등등 업무가 커짐에따라 엔터티가 추가된다.
...
- 제 N형 정규형을 만드는것을 제N 정규화 라고 한다.
  ...
  - #### 5NF(제5정규형)
  4NF를 실행했을때 나뉘어진 모든 엔터티들이 원래 엔터티의 하위키이며 더 분해할 경우 기존 정보가 손실되어 더이상 분해될수 없는 경우.
  조인종속성 이용
...
1. 테이블 반정규화
  - 테이블 수직분할
  엔터티의 일부 속성을 별도의 1:1관계의 엔터티로 분할
  예) 회원의 배우자명, 배우자핸드폰번호를 회원배우자 테이블로

  - 테이블 수평분할
  엔터티의 인스턴스를 특정 기준으로 별도의 엔터티의 분할
  데이터베이스의 파티셔닝 기법을 주로 이용.
  예) 주문테이블을 2020년 주문, 2021년 주문으로. 

  - 테이블 추가
  예) 중복 테이블 추가 : 데이터의 중복이 있더라도 성능상 반드시 필요하다고 판별되는 경우 별도의 엔터티를 추가
  통계 테이블 추가
  이력 테이블 추가
  부분 테이블 추가 : 메일발송이 다량으로 있을 경우 메일 발송에 필요한 정보만 회원에서 떼와서 테이블로 생성
...
3. 관계 반정규화(중복관계 추가)
  -JOIN이 필요한 경우가 많아 중복관계 추가하는것이 성능상 유리
...
예를들어 공정타입 어트리뷰트가 있고 공정제어 엔터티가 있으면,
...
신분이 학생인 사람이라면 엔터티지만
강좌를 신청한 사람이라면 엔터티가 아니라 릴레이션
-복잡한 업무를 단순하게 만드는것이 실력. 업무를 모르고도 리드 해서 결정해나갈수있다.
-키엔터티, 메인엔터티는 전체의 1/3이 절대 안넘는다. 골조다. 골조가 무너지면 다무너진다.
