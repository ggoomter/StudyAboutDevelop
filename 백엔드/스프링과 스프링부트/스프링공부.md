...
# 스프링의 구동과정(프로세스)
1. web.xml에서 클라이언트의 요청을 받는다.
2. web.xml에 있는 dispatcherServlet이 요청을 핸들링하여 servlet-context.xml로 가서 HandlerMapping Contrller 를 검색하여 요청을 처리할 컨트롤러에게 넘긴다.
[참고](http://egloos.zum.com/springmvc/v/504151#type=comment&page=3)
3. 요청을 받은 컨트롤러는 서비스로 요청을 처리하고 결과를 출력할 뷰의 이름을 알려준다.
4. viewResolver는 컨트롤러가 준 뷰이름이 있는지 확인하여 View에게 View를 준다.
5. View는 처리 결과 데이터가 포함된 뷰를 디스패처 서블릿에게 준다.
6. 디스패처서블릿은 클라이언트에게 결과를 반환한다.
...
- web.xml
   ...
   <context-param>
    ...
    > url패턴을 *.do로 하는건 고전시대 방식, /*로 하면 web.xml의 .jsp를 처리하는 설정파일대로 적용이 안되기 때문에 / 가 정석이다.
...
- root-context.xml
    ...
    여기있는 모든 bean들은 context에서 사용되어 공유가 가능하다.
    어플리케이션 영역을 설정하는 것이다.
...
1. # Annotation (어노테이션)
    ...
    - 종류
        ...
        - ### @RequestBody (**중요**)
            -  Http의 **Body를 통째로!!! 키,밸류가 있다면 그대로 자바객체로 변환**해서 매핑된 메소드 파라미터로 매핑해준다(Parsing).
            -  Json이나 Xml로 전송된 파라미터를 매핑할 때 사용한다.
            -  보내주는측에서 contentType: "application/json; UTF-8;" 을 해주어야 한다.
            -  ajax로 전달할때는 JSON.stringify(객체)를 통해 json으로 데이터를 변환해서 data에 넣어준다.
