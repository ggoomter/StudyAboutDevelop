...
스프링 수업의 반은 환경설정이다. 스프링 환경설정하면 반 했네 라고 할정도다.
...
- web.xml
    프로젝트가 구동할때 제일 처음 참조하는 설정파일. WAS 관련된 설정.
...
---
### 필터 vs 인터셉터
  - 비지니스 로직과 분리하여 선/후 처리
  ![spring mvc request life cycle](../../이미지/리퀘스트생명주기.png)
### filter
- Web Application에서 관리되는 영역.
- 요청/응답의 최초/최종 단계의 위치에 존재하기 때문에
  요청/응답 정보를 변경하거나 스프링에 의해서 데이터가 변환되기 전의 순수한 Client의 요청/응답 닶을 확인할 수 있다.(유일하게 ServletRequest, ServletResponse 의 객체를 변환할 수 있다.)
-
### Interceptor
