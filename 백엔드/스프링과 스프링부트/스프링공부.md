...
스프링이라는 이름의 기원 : 기존의 EJB로 대표되는 Enterprise Framework의 시대를 겨울로 정의하고, 이젠 봄이 왔다는 의미.


## 왜 스프링을 배워야하는가?
(왜 스프링)[https://12bme.tistory.com/157]
기업의 채용조건(자바로 백엔드를 개발한다고 하면 필수)
그럼 기업에서는 왜 스프링을 사용하는가?
  살아남은이유, 핵심철학, 특징3가지에 녹아있다.
간단하게 말하자면 자바 엔터프라이즈 개발을 편하게 해주는 오픈소스 경략급 어플리케이션 프레임워크 이기 때문.
쉽게말하면 제한된 자원(개발자, 시간, 돈)으로 상업용으로 쓸만한 앱(엔터프라이즈앱)을 여러명이 같이 개발하는것을 매우 수월하게 해주기 때문.
...
# 따라하기 좋은 사이트
- [MVC 기본설정1](https://kimvampa.tistory.com/158?category=843151)
- [MVC 기본설정2](https://kimvampa.tistory.com/159?category=843151)

  <details>
  <summary>jdbc연결 소스</summary>
  <div markdown = "1">
  ```java
  ...
  public class JDBCTest {
  	...
  	public void testConnection() {
          ...
          String USER = "ggoomter";
          String PASSWORD = "rmfpdlxm3";
...
- [MVC 기본설정3](https://kimvampa.tistory.com/162?category=843151)
- [게시판등록1](https://kimvampa.tistory.com/163?category=843151)
- [게시판등록2](https://kimvampa.tistory.com/164?category=843151)
- [게시판목록](https://kimvampa.tistory.com/165?category=843151)
...
1. # 스프링
  - ### 스프링의 핵심철학
    객체지향 프로그래밍이 제공하는 폭넓은 혜택을 누릴 수 있도록 기본으로 돌아가자. 그래서 가장 관심을 많이 두는 대상이 '오브젝트'다.
...
- ## IoC/DI  (생성과 제어)
    ...
    - ### DI. Dependency Injection. 의존관계 주입.
	  ...
	  - 쉽게 설명1
        의존을 꼭 필요한 부품이라고 생각하면된다. 스프링은 일체형부품을 조립형으로 만든것이 큰 특징이라는 것이다.
        느슨한 결합으로 부품만 갈아끼울수 있도록.   부품객체를 디펜던시라고 하고, 꽂아주는것을 주입이라고 하는것이다.
        ...
        public class Strudent{
            ...
            public Strudent(Teacher myTeacher){
                this.myTeacher = myTeacher;
            }
        }
        즉 선생님을 학생이 결정하는것이 아니라 학생 외부에서 결정하도록 하는것.
        학생에서 선생님을 결정하게 하면 문제의 근본적인 원인은 가져오는 클래스에 대해 너무 많이 알고있기 때문이다.
