- [진짜 정리 잘된 mysql, gradle 블로그](https://congsong.tistory.com/category/Spring%20Boot)
  ...
  - [로그백](23)	3페이지 제일밑 오른쪽
  - [인터셉터](https://congsong.tistory.com/24?category=749196)
    The type HandlerInterceptorAdapter is deprecated되어서 HandlerInterceptor를 implements 하는 방식으로 변경. prehandle 의 return true로 안바꾸면 무한 콜백
  - [AOP] (25)  뒷부분은 [트랜잭션]
...
[좋은 블로그] (https://catsbi.oopy.io/32a9458e-f452-4733-b87c-caba75f98e2d)
...
3.x버전부터는 인라인 표현식이 추가되어 html태그없이 표현이 가능하다.
th:inline="text"  태그 내부에서만 [[${값}]] 형태로 처리가능.
예)
```javascript
    <!-- 인라인 처리 -->
...
- 절차
  ...
  1. WebSecurityConfigurerAdapter를 상속한 SpringSecurityConfig등의 파일 생성
    ...
    - UserDetailService 클래스 bean 등록
* 구글에 jdbc authentication 검색

  1. Service 에 암호화 하려는곳에 passwordEncoder.encode 함수 사용
