...
#### th:속성
  ...
  - ★★ layout:fragment="프래그먼트명"     
    ...
    - **부품을 쓰는화면에서 쓰면** 해당이름부품을 자기가 쓰고있는 레이아웃에 끼워넣으라는 말
    - **레이아웃에서 쓰면** 이 위치에 끼워넣으라는 말
...
###### 자바스립트에서 사용
  - 순수 자바스크립트일때(정적웹사이트)
    var num = /*[[${변수명}]]*/;  
  - 인라인 형태일때(타임리프 쓴 동적사이트)
    var num = [[${변수명}]];   근데 주석넣어도 사용가능.
    <script th:inline="javascript"></script> 안에 넣는거 잊지말아야한다.
    인라인에서는 알아서 문자열일때 따옴표 추가된다. 
    객체면 알아서 JSON형태로  객체화해준다.
    인라인에서는 주석을 문법으로 활용한다.
