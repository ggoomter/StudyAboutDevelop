...
확인해보고 윈도우 자격증명 없애애고 git push 하면 깃헙로그인창 뜨는데 거기서 토큰으로 접속해라고 하는게 대부분의 답이지만 안됨.
- https://velog.io/@arthur/GitHub-%EC%97%90%EB%9F%AC-%ED%95%B4%EA%B2%B0-The-requested-URL-returned-error-403
- 세팅 : git remote set-url origin 복사한주소
	예) git remote set-url origin https://github.com/Kimsia0717/teamC_campYo.git
- remote주소 확인 : git remote -v
- 푸쉬 : git push -f origin 브랜치이름

소스트리의 오른쪽위 설정 - '원격' 버튼눌러보면 이 프로젝트를 가져온 원격 주소가 있다.
403 에러가 뜬다면 원래 거기서 편집 눌러서 url/경로 에 있는 https://github.com/Kimsia0717/teamC_campYo.git
이런식으로 되어있던 주소를  https://올바른아이디@github.com/Kimsia0717/teamC_campYo.git
로 바꿔주면 된다.
또는 소스트리의 설정 - 계정 에서 접속한 아이디 바꿔주면 사용자 바뀌기 때문에 자동으로 된다.
mac에서는 이게 잘 동작안된다고 한다.
