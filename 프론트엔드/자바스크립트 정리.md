...
- Javascript 객체 문법으로 구조화된 데이터를 표현하기 위한 문자 기반의 표준 포맷. 구조화된 데이터를 표현하는것이기 때문에 함수는 전달못한다.
- 객체이름{키, 밸류}
...
* 배열 내장함수
    ...
    splice(시작인덱스, 지울갯수, 추가데이터)  //원본이 변함. 갯수없으면 시작인덱스 뒤로 다지움. 추가데이터가 있다면 지운자리에 채움.
    concat(추가할배열)    //추가할배열을 기존의 뒤에합침

    //삭제
    pop()   //제일 뒤의 요소를 제거
    shift() //제잎 앞의 요소를 제거. 매우 느리다.
    splice(시작인덱스, 지울갯수) // 인덱스0부터 2개를 삭제. 원본이 삭제되기 때문에 거의 사용하지 않는다.
    ...
    sort()      //정렬. 원본도 바뀜
        [소트함수 api](https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Global_Objects/Array/sort)
            ...
            //숫자를 올바르게 정렬하려면 아래와 같이 콜백함수를 활용해야한다.
