...
- 변수의 선언과 초기화
    ...
    블록 유효범위를 무시하기 때문에 사용을 지양해야한다. 뒷통수를 많이 맞고 그것을 해결하기 위해 let이 탄생했다.
...
일시적인 사각지대. 변수가 스코프의 시작지점부터 초기화가 시작되는 지점까지의 구간.
let과 const는 TDZ의 영향을 받아 var보다 예측가능하게 하고 잠재적인 버그를 줄인다.

---
### 호이스팅 (hoisting .끌어올림)
스코프 내부 어디서든 변수 선언은 최상위에 선언된 것 처럼 행동하는 것
즉, 스코프 단위로 발생한다.
그런데 var는 함수레벨 스코프,  let, const는 블록 레벨 스코프를 가진다.
...
---
### 데이터타입. 자료형
어떤 프로그래밍 언어든 primitive(원시), object(객체) 2가지 타입이 있다. 원시형이 아니면 다 객체형이다.
왜 데이터타입을 이렇게 나눠놨습니까? 세세하게 메모리 관리를 가능하도록 하기위해. 메모리는 비싼자원이다. 일단 원시형 타입을 보고 객체타입은는 중요하기 때문에 후에 따로다룸.
- ☆☆☆ 자바스크립트의 유의할점 ☆☆
Dynamic Typed Language(다이나믹타입트 랭귀지).  데이터타입이 런타임때 결정됨.
...
6. scope    함수 범위, 블록 범위, 렉시컬(lexical) 범위
